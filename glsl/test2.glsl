...
void main() {
    ...
    if (any(greaterThan(abs(lipstick_texture_coordinate - vec2(0.5)), vec2(0.5)))) {
        ...
        vec4 testColor = texture2D(lipstick_texture, lipstick_texture_coordinate);
        
        float alpha = texture2D(lipstick_texture, lipstick_texture_coordinate).r * float(enable_lipstick); //lip mask
        ...
        testColor = vec4(contrast_mask, contrast_mask, contrast_mask, 1.0);
